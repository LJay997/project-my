...
web-lgb-v3（工人 mobileClockNew / 管理端 mobileClockManager）
  → GET /attendance/clock/timeWindow/check   # BFF 透传
  → attendance-service                        # 查规则 + 匹配 + L1
...
| | 工人端 | 管理端打卡页 |
|--|--------|----------------|
| 项目/班组/租户 | `clockProjectId` 等 → `x-project-id` / `x-group-id` / `x-tenant-id` | `getManagerClockRequestConfig()` 覆盖为**本页选中项**（与首页 `lgb-pre-*` 独立） |
| 工人 | token → `x-worker-id`（登录人） | 同左（本人，非代打） |
| 角色 | `WORKER` | 强制 `x-lgb-request-role=WORKER` |

BFF：`getProjectGroup()` + `projectWorkerId` 解析后 Feign；不缓存业务结果。
...
| 工人属性 | **不在本 SQL**；另查 `projectWorkerId` → `ClockMatchWorker`（company/group/team/job/postType） |
